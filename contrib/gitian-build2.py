...
# Copyright (c) 2019-2021 The DSW Core developers
...
def setup_repos():
    ...
    if not os.path.isdir('dsw-detached-sigs'):
    ...
    if args['docker-image-hash']:
        make_image_prog += ['--docker-image-hash',args['docker-image-hash']]
    elif not args.kvm:
        make_image_prog += ['--lxc']
...
def build():
    ...
    os.makedirs('dsw-binaries/' + args.version, exist_ok=True)
    ...
    if args.windows:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'dsw='+args.commit, '--url', 'dsw='+args.url, '../dsw/contrib/gitian-descriptors/gitian-win2.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-unsigned', '--destination', '../gitian.sigs/', '../dsw/contrib/gitian-descriptors/gitian-win2.yml'])
        #subprocess.check_call('mv build/out/__decenomy__-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/__decenomy__*-win64.tar.gz ../dsw-binaries/'+args.version, shell=True)

    if args.macos:
        print('\nCompiling ' + args.version + ' MacOS')
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'dsw='+args.commit, '--url', 'dsw='+args.url, '../dsw/contrib/gitian-descriptors/gitian-osx2.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-unsigned', '--destination', '../gitian.sigs/', '../dsw/contrib/gitian-descriptors/gitian-osx2.yml'])
        #subprocess.check_call('mv build/out/__decenomy__-*-osx-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/__decenomy__*-osx64.tar.gz ../dsw-binaries/'+args.version, shell=True)
...
def sign():
    ...
    #subprocess.check_call('cp inputs/dsw-' + args.version + '-win-unsigned.tar.gz inputs/dsw-win-unsigned.tar.gz', shell=True)
    #subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../dsw/contrib/gitian-descriptors/gitian-win-signer.yml'])
    #subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-win-signed', '--destination', '../gitian.sigs/', '../dsw/contrib/gitian-descriptors/gitian-win-signer.yml'])
    #subprocess.check_call('mv build/out/dsw-*win64-setup.exe ../dsw-binaries/'+args.version, shell=True)
    #subprocess.check_call('mv build/out/dsw-*win32-setup.exe ../dsw-binaries/'+args.version, shell=True)

    # TODO: Skip making signed windows sigs until we actually start producing signed windows binaries
    #print('\nSigning ' + args.version + ' MacOS')
    #subprocess.check_call('cp inputs/__decenomy__-' + args.version + '-osx-unsigned.tar.gz inputs/__decenomy__-osx-unsigned.tar.gz', shell=True)
    #subprocess.check_call(['bin/gbuild', '--skip-image', '--upgrade', '--commit', 'signature='+args.commit, '../dsw/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    #subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-osx-signed', '--destination', '../gitian.sigs/', '../dsw/contrib/gitian-descriptors/gitian-osx-signer.yml'])
    #subprocess.check_call('mv build/out/__decenomy__-osx-signed.dmg ../dsw-binaries/'+args.version+'/__decenomy__-'+args.version+'-osx.dmg', shell=True)
    ...
    if args.commit_files:
        os.chdir('gitian.sigs')
        commit = False
        if os.path.isfile(args.version+'-win-signed/'+args.signer+'/dsw-win-signer-build.assert.sig'):
            subprocess.check_call(['git', 'add', args.version+'-win-signed/'+args.signer])
            commit = True
        if os.path.isfile(args.version+'-osx-signed/'+args.signer+'/dsw-dmg-signer-build.assert.sig'):
...
def verify():
    ...
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../dsw/contrib/gitian-descriptors/gitian-win.yml']):
        print('Verifying v'+args.version+' Windows FAILED\n')
        rc = 1

    print('\nVerifying v'+args.version+' MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../dsw/contrib/gitian-descriptors/gitian-osx.yml']):
        print('Verifying v'+args.version+' MacOS FAILED\n')
        rc = 1

    # TODO: Skip checking signed windows sigs until we actually start producing signed windows binaries
    #print('\nVerifying v'+args.version+' Signed Windows\n')
    #if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../dsw/contrib/gitian-descriptors/gitian-win-signer.yml']):
    #    print('Verifying v'+args.version+' Signed Windows FAILED\n')
    #    rc = 1

    print('\nVerifying v'+args.version+' Signed MacOS\n')
    if subprocess.call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../dsw/contrib/gitian-descriptors/gitian-osx-signer.yml']):
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/zimbora/dsw', help='Specify the URL of the repository. Default is %(default)s')
    ...
    if args.pull:
        ...
        if not os.path.isdir('../gitian-builder/inputs/dsw'):
            os.makedirs('../gitian-builder/inputs/dsw')
        os.chdir('../gitian-builder/inputs/dsw')
